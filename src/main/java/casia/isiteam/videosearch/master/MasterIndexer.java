...
public class MasterIndexer {
	...
	public MasterIndexer(String configFilePath) throws IOException {
		...
		new FileReceiver(configuration.host, configuration.fileTransferPort,
				configuration.tempFileDir, false).start();

		// 发布注册服务
		URL url = new URL("http", configuration.host,
		...
		// 对外发布检索服务
		URL urlService = new URL("http", configuration.host,
				configuration.servicePort, "/"
						+ MasterIndexService.class.getSimpleName());
